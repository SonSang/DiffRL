...
for env in mpath.keys():
    ...
    axs[0, 0].plot(n_timesteps, alpha_mean, c=[1, 0, 0], linewidth=1)
    axs[0, 0].fill_between(n_timesteps, alpha_mean - alpha_std, alpha_mean + alpha_std, alpha=0.3, facecolor = [1, 0, 0])
    ...
    meandet_mean_spline = make_interp_spline(n_timesteps, meandet_mean)
    mindet_mean_spline = make_interp_spline(n_timesteps, mindet_mean)
    maxdet_mean_spline = make_interp_spline(n_timesteps, maxdet_mean)
    meandet_mean_spline_results = meandet_mean_spline(n_timesteps_spline)
    mindet_mean_spline_results = mindet_mean_spline(n_timesteps_spline)
    maxdet_mean_spline_results = maxdet_mean_spline(n_timesteps_spline)
    axs[1, 0].plot(n_timesteps_spline, meandet_mean_spline_results, c=[0.4940, 0.1840, 0.5560], label="Mean", linewidth=1)
    axs[1, 0].plot(n_timesteps_spline, mindet_mean_spline_results, c=[0.9290, 0.6940, 0.1250], label="Min", linewidth=1)
    axs[1, 0].plot(n_timesteps_spline, maxdet_mean_spline_results, c=[0, 0, 1], label="Max", linewidth=1)
    #axs[1].set_xlabel('Epoch')
    axs[1, 0].set_ylabel('Estimate Determinant')
    axs[1, 0].grid(True)
    axs[1, 0].legend()
    
    ecp_mean_spline = make_interp_spline(n_timesteps, ecp_mean)
    ecp_std_spline = make_interp_spline(n_timesteps, ecp_std)
    ecp_mean_spline_results = ecp_mean_spline(n_timesteps_spline)
    ecp_std_spline_results = ecp_std_spline(n_timesteps_spline)
    axs[0, 1].plot(n_timesteps_spline, ecp_mean_spline_results, c=[1, 0, 0], linewidth=1)
    axs[0, 1].fill_between(n_timesteps_spline, 
                           ecp_mean_spline_results - ecp_std_spline_results, 
                           ecp_mean_spline_results + ecp_std_spline_results, 
                           alpha=0.3, facecolor = [1, 0, 0])
    axs[0, 1].set_ylabel('Estimated current performance')
    axs[0, 1].grid(True)
    
    axs[1, 1].plot(n_timesteps, oor_mean, c=[1, 0, 0], linewidth=1)
    axs[1, 1].fill_between(n_timesteps, oor_mean - oor_std, oor_mean + oor_std, alpha=0.3, facecolor = [1, 0, 0])
    axs[1, 1].set_ylabel('Out of Range Ratio')
    axs[1, 1].grid(True)
