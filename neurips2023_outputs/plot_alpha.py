...
for env in mpath.keys():
    ...
    axs[1, 0].plot(n_timesteps, meandet_mean, c=[0.4940, 0.1840, 0.5560], label="Mean", linewidth=1)
    axs[1, 0].plot(n_timesteps, mindet_mean, c=[0.9290, 0.6940, 0.1250], label="Min", linewidth=1)
    axs[1, 0].plot(n_timesteps, maxdet_mean, c=[0, 0, 1], label="Max", linewidth=1)
    #axs[1].set_xlabel('Epoch')
    axs[1, 0].set_ylabel('Estimate Determinant')
    axs[1, 0].grid(True)
    axs[1, 0].legend()
    
    axs[0, 1].plot(n_timesteps, ecp_mean, c=[1, 0, 0], linewidth=1)
    axs[0, 1].fill_between(n_timesteps, ecp_mean - ecp_std, ecp_mean + ecp_std, alpha=0.3, facecolor = [1, 0, 0])
