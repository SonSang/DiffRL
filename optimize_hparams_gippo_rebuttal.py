...
def objective(trial):
    ...
    # e_clip = float(trial.suggest_categorical("e_clip", ["0.2", "0.3", "0.4", "0.5"]))
    # cfg_train['params']['config']['e_clip'] = float(e_clip)
    cfg_train['params']['config']['gi_params']['stable_ppo'] = False
    cfg_train["params"]["config"]["gi_params"]["max_dist_rp_lr"] = 1.
...
if __name__ == '__main__':
    ...
    args.logdir = f"optuna/logs/gippo_rebuttal_2/{args.env}/"
    args.cfg = f"./neurips2023_outputs/physics/cfg/gippo/{args.env}.yaml" # f"./examples/cfg/grad_ppo_alpha/{args.env}.yaml"
    ...
    study_name = f"gippo-rebuttal-{args.env}-2"  # Unique identifier of the study.
